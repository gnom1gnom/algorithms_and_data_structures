...
bool isMyPermutationOfPallindrome(const std::string &str)
{
    // cout << std::endl;
    bool result = false;
    ...
    {
        // cout << str[i] << std::endl << "zyxwvutsrqponmlkjihgfedcba" << std::endl;
        // cout << (bitset<26>)lettersVector << std::endl;

        // skip space
        if (str[i] == ' ')
            continue;
        int letterCode = (int)(str[i] - 'a');
        int letterAsVector = 1 << letterCode;

        // cout << (bitset<26>)letterAsVector << std::endl;

        lettersVector = lettersVector ^ letterAsVector;
        // cout << (bitset<26>)lettersVector << std::endl;
    }

    // sprawdzamy czy liczba jest potegą 2 - czyli ze jest tam tylko jedna 1
    return !(lettersVector & (lettersVector - 1));
}
...
int main()
{
    ...
    std::cout << "Approach My:" << isMyPermutationOfPallindrome(str) << std::endl;
    std::cout << "Approach:" << isPermutationOfPallindrome1(str) << std::endl;

    std::string str1("A big Cat");
    std::cout << "Does \"" << str1 << "\" has a string whose permutation is a pallindrome? "
              << "( 1 for true, 0 for false ) : " << std::endl;
    std::cout << "Approach My:" << isMyPermutationOfPallindrome(str1) << std::endl;
    std::cout << "Approach:" << isPermutationOfPallindrome1(str1) << std::endl;

    std::string str2("Aba cbc");
    std::cout << "Does \"" << str2 << "\" has a string whose permutation is a pallindrome? "
              << "( 1 for true, 0 for false ) : " << std::endl;
    std::cout << "Approach My:" << isMyPermutationOfPallindrome(str2) << std::endl;
    std::cout << "Approach:" << isPermutationOfPallindrome3(str2) << std::endl;
    return 0;
}
