...
/**
 * @brief Get the Kth To The Last object
 * Tak naprawdę ta rekursja zwraca tylko numer elementu od końca
 * 
 * @param currentnode 
 * @param kthNode 
 * @param kth 
 * @return int 
 */
int getKthToTheLast(Node *currentnode, Node *&kthNode, int kth)
{
  if (!currentnode)
    return 0;

  int nodeNoFromBottom = getKthToTheLast(currentnode->next, kthNode, kth);
  nodeNoFromBottom++;
  if (kth == nodeNoFromBottom)
  {
    kthNode == currentnode;
  }
  return nodeNoFromBottom;
}
...
int main()
{
  ...
  std::cout << "4th node from last (My Recursive) : ";
  Node *kth = nullptr;
  getKthToTheLast(head, kth, 4);
  if (node4 != nullptr)
  {
    std::cout << node4->data << std::endl;
  }
